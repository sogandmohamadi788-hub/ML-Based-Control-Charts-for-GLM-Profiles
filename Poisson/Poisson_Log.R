...
# Function to generate Poisson GLM profile data with log link
...
SVR_poisslog <- matrix(NA,3,5)
colnames(SVR_poisslog) <- c("Type","RMSE","ARL","SDRL","UCL")
SVR_poisslog[1,1] <- "Eps linear"
SVR_poisslog[2,1] <- "Eps radial"

set.seed(111)
gen_ucl=poiss_log(n=10,shift0=0,shift1=0,number_iter=10000) 
...
for (k in 1:3000) {
  ...
  while (yhat<ucl_SVR) {
    rl=rl+1
    x_NN_new=poiss_log(n=10,shift0=s1,shift1=s2)
...
SVR_poisslog[3,] <- c("Eps linear",RMSE,mean(arl),sd(arl),ucl_SVR)
RMSE=rmse(SVR$fitted,input_NN[,4])
SVR_poisslog <- rbind(SVR_poisslog, c("Eps linear",RMSE,mean(arl),sd(arl),ucl_SVR))

write.csv(SVR_poisslog, "C:/Users/patri/OneDrive/Desktop/Master_thesis/Output/SVR_poisslog_combo.csv", row.names=FALSE)


SVR_poisslog_shift <- matrix(NA,22,7)
colnames(SVR_poisslog_shift) <- c("Type","Error","ARL","SDRL","UCL","Shift 1","Shift 2")
...
for (y in 1:22) {
  ...
  SVR_poisslog_shift[5,]<-c("Eps linear",SVR_poisslog[1,2],mean(arl),sd(arl),ucl_SVR,s1,s2)
}

write.csv(SVR_poisslog_shift, "C:/Users/patri/OneDrive/Desktop/Master_thesis/Output/SVR_poisslog_combo_linear_shift.csv", row.names=FALSE)


RF_poisslog <- matrix(NA,60,5)

colnames(RF_poisslog) <- c("Number of trees","MSE","ARL","SDRL","UCL")
set.seed(111)
for (i in 1:60) {
  RF=randomForest(Y~ .,data=input_NN,ntree=i,importance=FALSE)
  RF_poisslog[i,2] <-mean(RF$mse)
  RF_poisslog[i,1] <- i
}
...
RF_poisslog[44,c(3,4,5)]<- c(mean(arl),sd(arl),ucl_RF)
RF_poisslog <- rbind(RF_poisslog, c(54,RF_poisslog[54,2],mean(arl),sd(arl),ucl_RF))


write.csv(RF_poisslog, "C:/Users/patri/OneDrive/Desktop/Master_thesis/Output/RF_poisslog.csv", row.names=FALSE)

NN_poisslog <- matrix(NA,60,6)

colnames(NN_poisslog) <- c("Nodes","Iterations","RMSE","ARL","SDRL","UCL")
...
for (i in 1:60) {
  ...
  NN_poisslog[i,1] <- i
  NN_poisslog[i,2] <- max_iter
  NN_poisslog[i,3] <- RMSE
...
NN_poisslog[49,c(4,5,6)] <-c(mean(arl),sd(arl),ucl_NN)
NN_poisslog = rbind(NN_poisslog,c(49,1000,NN_poisslog[49,3],mean(arl),sd(arl),ucl_NN))

write.csv(NN_poisslog, "C:/Users/patri/OneDrive/Desktop/Master_thesis/Output/NN_poisslog.csv", row.names=FALSE)


NN_poisslog[51,c(4,5,6)] <-c(mean(arl),sd(arl),ucl_NN)
